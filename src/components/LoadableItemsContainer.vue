<template>
  <div class="loadable-container">
    <ScrollableBlock ref="elemScrollableBlock">
...
export default {
  ...
  setup(props) {
    ...
    const isIntersectionInVision = () => {
      const elemScrollableBlockScroll = elemScrollableBlock.value.elemScrollableBlock.querySelector('.ss-content')
      const rectScrollBlock = elemScrollableBlockScroll.getBoundingClientRect()
      const rectIntersection = elemIntersection.value.getBoundingClientRect()

      const minDistance = elemScrollableBlockScroll.scrollTop + rectScrollBlock.y + rectScrollBlock.height
